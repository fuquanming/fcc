...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			url : '<%=basePath%>manage/sys/indexTask/report/datagrid.do?random=' + Math.random(),
			toolbar : '#toolbar',
			title : '',
			iconCls : 'icon-save',
			pagination : true,
...
<div region="center" border="false">
  <div id="toolbar" class="datagrid-toolbar" style="height: auto;display: none;">
		...
		<tr>
			...
			<td colspan="2">
				<select name="reportGroupName" id="reportGroupName" style="width: 130px;">
					...
					<option value="operation"><%=IndexTask.ALIAS_OPERATION%></option>
					<option value="documentId"><%=IndexTask.ALIAS_DOCUMENT_ID%></option>
					<option value="documentType"><%=IndexTask.ALIAS_DOCUMENT_TYPE%></option>
					<option value="createTime"><%=IndexTask.ALIAS_CREATE_TIME%></option>
				</select>
			</td>
		</tr>	
		<tr>
    ...
    <div> 
    <a class="easyui-linkbutton" iconCls="icon-search" onClick="lineCharts('<%=IndexTask.TABLE_ALIAS%>', '<%=basePath %>');" plain="true" href="javascript:void(0);">折线图</a>
    <a class="easyui-linkbutton" iconCls="icon-search" onClick="columnCharts('<%=IndexTask.TABLE_ALIAS%>', '<%=basePath %>');" plain="true" href="javascript:void(0);">柱形图</a>
    <a class="easyui-linkbutton" iconCls="icon-search" onClick="pieCharts('<%=IndexTask.TABLE_ALIAS%>', '<%=basePath %>');" plain="true" href="javascript:void(0);">饼图</a> |
    </div>
  </div>
