...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			<c:if test="${empty param.lazy}">url : '<%=basePath%>manage/sys/indexTask/datagrid.do?random=' + Math.random(),</c:if>
			toolbar : '#toolbar',
			title : '',
			iconCls : 'icon-save',
			pagination : true,
			...
			columns : [ [ 
				{
					field : 'operation',
					title : '<%=IndexTask.ALIAS_OPERATION%>',
					sortable:true,
					width : 100
				} ,
				{
					field : 'documentId',
					title : '<%=IndexTask.ALIAS_DOCUMENT_ID%>',
					sortable:true,
					width : 100
				} ,
				{
					field : 'documentType',
					title : '<%=IndexTask.ALIAS_DOCUMENT_TYPE%>',
					sortable:true,
					width : 100
				} ,
				{
					field : 'createTime',
					title : '<%=IndexTask.ALIAS_CREATE_TIME%>',
					sortable:true,
					width : 100,
		...
		$("#file_upload").uploadify({ 
	        ...
	        'uploader':'<%=basePath%>manage/sys/indexTask/import.do?JSESSIONID=<%=session.getId()%>', 
	        //浏览按钮的背景图片路径 
	...
	function view() {
		...
		if (rows.length != 1 && rows.length != 0) {
			...
			window.location.href = "<%=basePath%>manage/sys/indexTask/toView.do?id=" + rows[0].id;
		}
	}

	function add() {
		window.location.href = "<%=basePath%>manage/sys/indexTask/toAdd.do";
	}
	...
	function edit() {
		...
		if (rows.length != 1 && rows.length != 0) {
			...
			window.location.href = "<%=basePath%>manage/sys/indexTask/toEdit.do?id=" + rows[0].id;
		}
	}
			...
			$.messager.confirm('请确认', '您要删除当前所选记录？', function(r) {
				if (r) {
					...
					userForm.form('submit', {
						url : '<%=basePath%>manage/sys/indexTask/delete.do',
						onSubmit : function() {
							return true;
						},
	...
	function searchFun() {
		<c:if test="${not empty param.lazy}">datagrid = $('#datagrid').datagrid({url : '<%=basePath%>manage/sys/indexTask/datagrid.do?random=' + Math.random(),});</c:if>
		datagrid.datagrid('load', {
				operation : $('#toolbar input[name=operation]').val(),
				documentId : $('#toolbar input[name=documentId]').val(),
	...
	function exportData() {
		...
		userForm.form('submit', {
			url : '<%=basePath%>manage/sys/indexTask/export.do',
			success : function(data) {
				try {
					var d = $.parseJSON(data);
					$.messager.show({
	...
	function searchExportDataSize() {
		$.ajax({
			url : '<%=basePath%>manage/sys/indexTask/queryExport.do?random=' + Math.random(),
			cache : false,
			dataType : "json",
			success : function(d) {
				try {
					if (d.empty == true) {
						...
						$('#expordDataSizeSpan').html("已导出：" + d.currentSize + "条，导出完成！若没有提示下载，请点击 <a href='" + '<%=basePath%>exportData/indexTaskExport/' + d.fileName + "'>" + d.fileName + "</a>");
						exportDataFlag = false;
						window.location.href = '<%=basePath%>exportData/indexTaskExport/' + d.fileName;
	...
	function searchImportDataSize() {
		$.ajax({
			url : '<%=basePath%>manage/sys/indexTask/queryImport.do?random=' + Math.random(),
			cache : false,
			dataType : "json",
			success : function(d) {
				try {
	...
	function report()　{
		window.location.href = "<%=basePath%>manage/sys/indexTask/report/view.do";
	}
</script>
</head>
  ...
  <div id="toolbar" class="datagrid-toolbar" style="height: auto;display: none;">
    ...
    <table class="tableForm">
		<tr>	
			...
			<td colspan="2">
				<input id="operation" name="operation" maxlength="255"  style="width: 120px;"/>
			</td>
			<th><%=IndexTask.ALIAS_DOCUMENT_ID%></th>	
			<td colspan="2">
				<input id="documentId" name="documentId" maxlength="255"  style="width: 120px;"/>
			</td>
			<th><%=IndexTask.ALIAS_DOCUMENT_TYPE%></th>	
			<td colspan="2">
				<input id="documentType" name="documentType" maxlength="100"  style="width: 120px;"/>
			</td>
