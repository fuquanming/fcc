...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			<c:if test="${empty param.lazy}">url : '<%=basePath%>manage/sys/amountApply/workflow/processHistory/datagrid.do?random=' + Math.random(),</c:if>
			toolbar : '#toolbar',
			title : '',
			iconCls : 'icon-save',
			pagination : true,
			...
			columns : [ [ 
				...
				{
					field : 'processHistoryInfo.startTime',
					title : '流程启动时间',
					sortable:true,
					width : 100,
					formatter : function(value, rowData, rowIndex) {
						value = rowData.processHistoryInfo.startTime
						return Tool.dateFormat({'value' : value, 'format' : 'yyyy-MM-dd HH:mm:ss'});
					}
				} ,
				{
					field : 'processHistoryInfo.endTime',
					title : '流程结束时间',
					sortable:true,
					width : 100,
					formatter : function(value, rowData, rowIndex) {
						value = rowData.processHistoryInfo.endTime
						return Tool.dateFormat({'value' : value, 'format' : 'yyyy-MM-dd HH:mm:ss'});
					}
				} ,
				{
					field : 'processHistoryInfo.deleteReason',
					title : '流程结束原因',
					sortable:true,
					width : 100,
					formatter : function(value, rowData, rowIndex) {
						value = rowData.processHistoryInfo.deleteReason;
						return (value == null || "" == value) ? '正常结束' : value;
					}
				} ,
				{
					field : 'processHistoryInfo.processDefinitionVersion',
					title : '流程版本',
					sortable:true,
					width : 100,
					formatter : function(value, rowData, rowIndex) {
						return rowData.processHistoryInfo.processDefinitionVersion;
					}
				} 
			] ],
...
<div region="center" border="false">
  <div id="toolbar" class="datagrid-toolbar" style="height: auto;display: none;">
    ...
    <div id="userDialog" style="margin: 0 auto; overflow: hidden;">
		<img id="processInstanceImg" name="processInstanceImg" src=""/>
	</div>
    <div> 
