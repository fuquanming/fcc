...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			<c:if test="${empty param.lazy}">url : '<%=basePath%>manage/sys/amountApply/datagrid.do?random=' + Math.random(),</c:if>
			toolbar : '#toolbar',
			title : '',
			iconCls : 'icon-save',
			pagination : true,
		...
		$("#file_upload").uploadify({ 
	        ...
	        'uploader':'<%=basePath%>manage/sys/amountApply/import.do?JSESSIONID=<%=session.getId()%>', 
	        //浏览按钮的背景图片路径 
	...
	function start() {
		...
		if (rows.length > 0) {
			$.messager.confirm('请确认', '您要 启动 当前所选记录？', function(r) {
				if (r) {
					for ( var i = 0; i < rows.length; i++) {
						ids.push(rows[i].amountApplyId);
					}
					var idsVal = ids.join(',');
					userForm.find('[name=ids]').val(idsVal);
					userForm.form('submit', {
						url : '<%=basePath%>manage/sys/amountApply/workflow/start.do',
						onSubmit : function() {
							return true;
						},
			...
			$.messager.alert('提示', '请选择要 启动 的记录！', 'error');
		}
	}
	...
	function add() {
		window.location.href = "<%=basePath%>manage/sys/amountApply/toAdd.do";
	}
	...
	function edit() {
		...
		if (rows.length != 1 && rows.length != 0) {
			...
			window.location.href = "<%=basePath%>manage/sys/amountApply/toEdit.do?id=" + rows[0].amountApplyId;
		}
	}
		...
		if (rows.length > 0) {
			$.messager.confirm('请确认', '您要删除当前所选记录？', function(r) {
				if (r) {
					...
					userForm.find('[name=ids]').val(idsVal);
					userForm.form('submit', {
						url : '<%=basePath%>manage/sys/amountApply/delete.do',
						onSubmit : function() {
							return true;
						},
						success : function(data) {
	...
	function exportData() {
		...
		userForm.form('submit', {
			url : '<%=basePath%>manage/sys/amountApply/export.do',
			success : function(data) {
				try {
					var d = $.parseJSON(data);
					$.messager.show({
	...
	function searchExportDataSize() {
		$.ajax({
			url : '<%=basePath%>manage/sys/amountApply/queryExport.do?random=' + Math.random(),
			cache : false,
			dataType : "json",
			success : function(d) {
				try {
					if (d.empty == true) {
						...
						$('#expordDataSizeSpan').html("已导出：" + d.currentSize + "条，导出完成！若没有提示下载，请点击 <a href='" + '<%=basePath%>exportData/amountApplyExport/' + d.fileName + "'>" + d.fileName + "</a>");
						exportDataFlag = false;
						window.location.href = '<%=basePath%>exportData/amountApplyExport/' + d.fileName;
	...
	function searchImportDataSize() {
		$.ajax({
			url : '<%=basePath%>manage/sys/amountApply/queryImport.do?random=' + Math.random(),
			cache : false,
			dataType : "json",
			success : function(d) {
				try {
	...
	function report()　{
		window.location.href = "<%=basePath%>manage/sys/amountApply/report/view.do";
	}
</script>
</head>
