...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			<c:if test="${empty param.lazy}">url : '<%=basePath%>manage/sys/indexInfo/datagrid.do?random=' + Math.random(),</c:if>
			toolbar : '#toolbar',
			title : '',
			iconCls : 'icon-save',
			pagination : true,
			...
			columns : [ [ 
				{
					field : 'infoKey',
					title : '<%=IndexInfo.ALIAS_INFO_KEY%>',
					sortable:true,
					width : 100
				} ,
				{
					field : 'indexDir',
					title : '<%=IndexInfo.ALIAS_INDEX_DIR%>',
					sortable:true,
					width : 100
				} ,
				{
					field : 'documentType',
					title : '<%=IndexInfo.ALIAS_DOCUMENT_TYPE%>',
					sortable:true,
					width : 100
				} ,
				{
					field : 'className',
					title : '<%=IndexInfo.ALIAS_CLASS_NAME%>',
					sortable:true,
					width : 100
				} ,
				{
					field : 'analyzerName',
					title : '<%=IndexInfo.ALIAS_ANALYZER_NAME%>',
					sortable:true,
					width : 100
				} ,
				{
					field : 'createTime',
					title : '<%=IndexInfo.ALIAS_CREATE_TIME%>',
					sortable:true,
					width : 100,
		...
		$("#file_upload").uploadify({ 
	        ...
	        'uploader':'<%=basePath%>manage/sys/indexInfo/import.do?JSESSIONID=<%=session.getId()%>', 
	        //浏览按钮的背景图片路径 
	...
	function view() {
		...
		if (rows.length != 1 && rows.length != 0) {
			...
			window.location.href = "<%=basePath%>manage/sys/indexInfo/toView.do?id=" + rows[0].infoId;
		}
	}

	function add() {
		window.location.href = "<%=basePath%>manage/sys/indexInfo/toAdd.do";
	}
	...
	function edit() {
		...
		if (rows.length != 1 && rows.length != 0) {
			...
			window.location.href = "<%=basePath%>manage/sys/indexInfo/toEdit.do?id=" + rows[0].infoId;
		}
	}
			...
			$.messager.confirm('请确认', '您要删除当前所选记录？', function(r) {
				if (r) {
					...
					userForm.form('submit', {
						url : '<%=basePath%>manage/sys/indexInfo/delete.do',
						onSubmit : function() {
							return true;
						},
	...
	function searchFun() {
		<c:if test="${not empty param.lazy}">datagrid = $('#datagrid').datagrid({url : '<%=basePath%>manage/sys/indexInfo/datagrid.do?random=' + Math.random()});</c:if>
	...
	function exportData() {
		...
		userForm.form('submit', {
			url : '<%=basePath%>manage/sys/indexInfo/export.do',
			success : function(data) {
				try {
					var d = $.parseJSON(data);
					$.messager.show({
	...
	function searchExportDataSize() {
		$.ajax({
			url : '<%=basePath%>manage/sys/indexInfo/queryExport.do?random=' + Math.random(),
			cache : false,
			dataType : "json",
			success : function(d) {
				try {
					if (d.empty == true) {
						...
						$('#expordDataSizeSpan').html("已导出：" + d.currentSize + "条，导出完成！若没有提示下载，请点击 <a href='" + '<%=basePath%>exportData/indexInfoExport/' + d.fileName + "'>" + d.fileName + "</a>");
						exportDataFlag = false;
						window.location.href = '<%=basePath%>exportData/indexInfoExport/' + d.fileName;
	...
	function searchImportDataSize() {
		$.ajax({
			url : '<%=basePath%>manage/sys/indexInfo/queryImport.do?random=' + Math.random(),
			cache : false,
			dataType : "json",
			success : function(d) {
				try {
	...
	function report()　{
		window.location.href = "<%=basePath%>manage/sys/indexInfo/report/view.do";
	}
</script>
</head>
